...
\paragraph{Topics.} (1) the IMU measurement model (gyroscope, accelerometer, biases, noise); (2) continuous vs.\ discrete noise; (3) the IMU kinematics as a first-order ODE; (4) the big picture: IMU integration in a visual-inertial navigation system; (5) exact integration from $t_k$ to $t_{k+1}$; (6) computing $\Delta\vect{R},\Delta\vect{p},\Delta\vect{v}$ --- preintegration and three methods; (7) mean and covariance propagation with $SO(3)+\mathbb{R}^{3}$ errors; (8) the linearization using $SE_2(3)$.
...
\end{align}
which admit closed-form (analytic) solutions. We now use this form for the detailed mean and covariance propagation.
