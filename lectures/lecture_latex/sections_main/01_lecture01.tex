...
p(x) &= \lim_{\varepsilon\to 0}\frac{F_{\mathrm{x}}(x)-F_{\mathrm{x}}(x-\varepsilon)}{\varepsilon} \\
     &= \frac{\mathrm{d}F_{\mathrm{x}}(x)}{\mathrm{d}x} = \frac{\mathrm{d}P(\mathrm{x}\le x)}{\mathrm{d}x},
\end{align}
hence
\begin{align}
...
\begin{align}
m_i\!\iint x_j\, p(x_i,x_j)\,\mathrm{d}x_i\mathrm{d}x_j = m_i\!\int x_j\, p(x_j)\,\mathrm{d}x_j = m_i m_j,
\end{align}
...
\begin{align}
\E(x_i x_j) - \E(x_i)\E(x_j) = \iint x_i x_j\,\big(p(x_i,x_j) - p(x_i)p(x_j)\big)\,\mathrm{d}x_i\mathrm{d}x_j = 0,
\end{align}
...
q &= (\vect{x}-\vect{m}_x)\trans \vect{L}^{-\top}\vect{L}^{-1}(\vect{x}-\vect{m}_x) \\
  &= \vect{y}\trans\vect{y} = \sum_{i=1}^n y_i^2.
\end{align}
The whitened variable has zero mean and identity covariance,
\begin{align}
\vect{m}_y &= \E(\vect{y}) = \vect{L}^{-1}\big(\E(\vect{x}) - \vect{m}_x\big) = \vect{0}, \\
...
&= \vect{L}^{-1}\vect{L}\vect{L}\trans\vect{L}^{-\top} = \vect{I}_n,
\end{align}
so $y_i\sim\mathcal{N}(0,1)$ are independent. Therefore the mean of $q$ is
\begin{align}
\E(q) = \E(\vect{y}\trans\vect{y}) = \sum_{i=1}^n \E(y_i^2) = n.
\end{align}
For the variance, with $\E(y_i^2)=1$ the cross terms vanish by independence, giving
\begin{align}
\sigma_{qq}^2 &= \E\big((q - n)^2\big) = \E\Big(\big(\textstyle\sum_i (y_i^2-1)\big)^2\Big) \\
