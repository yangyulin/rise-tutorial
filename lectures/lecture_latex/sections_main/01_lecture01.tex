...
\item probability: conditional probability, the total probability theorem, Bayes' theorem, and independence;
\item random variables: distribution, probability density function, and the conditional / total-probability / Bayes relations for pdfs;
\item expectation, variance, covariance, and random vectors --- with the worked identities and the independence-vs-uncorrelatedness discussion;
\item maximum likelihood (MLE) and maximum a posteriori (MAP) estimation --- the weighted least-squares solution;
\item linear systems: the state transition matrix and the full LTV solution.
\end{itemize}
...
\paragraph{Homework: variance identity.}
...
\paragraph{Homework: scalar covariance identity.}
...
\paragraph{Solution: independence versus uncorrelatedness.}
...
\paragraph{Homework: covariance matrix identity.}
...
\paragraph{Solution: the \texorpdfstring{$\chi^2$}{chi-squared} distribution.}
Factor $\vect{P}_{xx} = \vect{L}\vect{L}\trans$ (Cholesky), so $\vect{P}_{xx}^{-1} = \vect{L}^{-\top}\vect{L}^{-1}$, and define the whitened variable $\vect{y} = \vect{L}^{-1}(\vect{x}-\vect{m}_x)$. Then
\begin{align}
