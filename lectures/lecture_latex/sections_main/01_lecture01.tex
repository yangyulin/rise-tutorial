...
\begin{align}
\sigma_x^2 = \E\big((\mathrm{x}-m_x)^2\big) = \E\big(\mathrm{x}^2 - 2 m_x \mathrm{x} + m_x^2\big)
= \E(\mathrm{x}^2) - 2 m_x \E(\mathrm{x}) + m_x^2 = \E(\mathrm{x}^2) - m_x^2.
\end{align}
...
\begin{align}
\sigma_{ij} = \E(x_i x_j) - m_i m_j - m_i m_j + m_i m_j = \E(x_i x_j) - \E(x_i)\E(x_j).
\end{align}
...
= \int x_i\, p(x_i)\,\mathrm{d}x_i \cdot \int x_j\, p(x_j)\,\mathrm{d}x_j = \E(x_i)\E(x_j),
\end{align}
so $\sigma_{ij}=0$.
...
\begin{align}
q = (\vect{x}-\vect{m}_x)\trans \vect{L}^{-\top}\vect{L}^{-1}(\vect{x}-\vect{m}_x) = \vect{y}\trans\vect{y} = \sum_{i=1}^n y_i^2.
\end{align}
The whitened variable has zero mean and identity covariance,
\begin{align}
\vect{m}_y &= \E(\vect{y}) = \vect{L}^{-1}\big(\E(\vect{x}) - \vect{m}_x\big) = \vect{0}, \\
\vect{P}_{yy} &= \E(\vect{y}\vect{y}\trans) = \vect{L}^{-1}\,\E\big((\vect{x}-\vect{m}_x)(\vect{x}-\vect{m}_x)\trans\big)\,\vect{L}^{-\top}
= \vect{L}^{-1}\vect{P}_{xx}\vect{L}^{-\top} = \vect{L}^{-1}\vect{L}\vect{L}\trans\vect{L}^{-\top} = \vect{I}_n,
\end{align}
...
= \sum_i \E\big((y_i^2-1)^2\big) = \sum_i \big(\E(y_i^4) - 2\E(y_i^2) + 1\big).
\end{align}
