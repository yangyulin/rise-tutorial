...
\item The EKF is a \emph{special form} of the MLE/MAP estimator.
\item The EKF is the way to solve the problem \emph{incrementally}.
\item The VINS system has exactly the structure of \eqref{eq:lec06-system}, repeated over time:
\begin{align}
\left.
\begin{cases}
\vect{x}_k \sim \mathcal{N}(\hat{\vect{x}}_k, \vect{P}_k) & \to\; \text{prior}, \\[2pt]
\vect{x}_{k+1} = \vect{f}(\vect{x}_k) + \vect{w}_k & \to\; \text{motion, IMU}, \\[2pt]
\vect{z}_{k+1} = \vect{h}(\vect{x}_{k+1}) + \vect{n}_{k+1} & \to\; \text{measurements, vision}
\end{cases}
\right\} \;\; \text{VINS}.
\label{eq:lec06-vins}
\end{align}
...
\item IMU motion model (Lecture~5): $\dot{\vect{x}} = \vect{f}(\vect{x}, \vect{n}_I) \;\Rightarrow\; \vect{x}_{k+1} = \vect{f}_{\mathrm{int}}(\vect{x}_k, \vect{n}_{Id})$;
\item vision model (Lectures~3--4): $\vect{z}_c = \vect{h}_c(\vect{x}) + \vect{n}_c$.
\end{itemize}
\item \textbf{Initialization:} in order to run the EKF, we need the prior $\vect{x}_k \sim \mathcal{N}(\hat{\vect{x}}_k, \vect{P}_k)$. For the IMU state (Lecture~5)
\begin{align}
\vect{x}_I = \big\{ {}^{G}_{I}\vect{R},\; {}^{G}\vect{p}_I,\; {}^{G}\vect{v}_I,\; \vect{b}_g,\; \vect{b}_a \big\},
\end{align}
we need to estimate ${}^{G}\vect{v}_I$, $\vect{b}_g$, $\vect{b}_a$ to start the estimator.
\end{enumerate}
...
\end{empheq}
\endgroup
Solving this linear system recovers the scale, the gravity direction in $\{I_k\}$ (hence the initial orientation up to yaw), and the velocities --- the quantities we needed to start the estimator.

\paragraph{How many frames do we need?} Each image pair contributes one position and one velocity constraint ($6$ equations); each new frame adds one velocity unknown ($3$):
\begin{itemize}[noitemsep,topsep=2pt]
\item \textbf{if no scale $s$} (e.g., stereo, where the vision translation is metric): with $3$ frames, $12$ equations vs.\ $12$ unknowns (${}^{I_k}\vect{g}$ + three velocities) $\Rightarrow$ we need at least $3$ frames;
\item \textbf{if scale $s$}: with $3$ frames, $12$ equations vs.\ $13$ unknowns $\Rightarrow$ we need at least $4$ frames.
\end{itemize}
