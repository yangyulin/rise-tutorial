...
\paragraph{Further reading.} This lecture draws on several standard references: matrix Lie groups in Barfoot~\cite[Ch.~7]{Barfoot2017StateEstimation}; the Lie-group treatment of $SO(3)$/$SE(3)$ in the GTSAM notes~\cite{DellaertGTSAMLieGroups} and Atanasov's ECE276A slides~\cite{Atanasov2020SO3SE3}; and quaternion algebra/kinematics in Trawny \& Roumeliotis~\cite[Sec.~1]{Trawny2005Quaternion} and Sol\`a~\cite[Sec.~1--3]{Sola2017Quaternion}.

% =====================================================================
\subsection{Preliminaries: useful series}
The matrix exponential and the sine/cosine series underpin everything that follows:
\begin{align}
e^{x} = \exp(x) &= \sum_{k=0}^{\infty}\frac{1}{k!}x^{k} = I + x + \tfrac{1}{2!}x^{2} + \tfrac{1}{3!}x^{3} + \tfrac{1}{4!}x^{4} + \cdots, \\
...
\begin{align}
{}^{A}\vect{L} = {}^{A}_{B}\vect{R}\,{}^{B}\vect{L} = {}^{A}_{B}\vect{R}(\theta_{A\to B})\,{}^{B}\vect{L}.
\end{align}
...
\subsubsection{Infinitesimal rotation and the skew-symmetric matrix}
...
\begin{align}
\skewsym{\vect{k}}^{2} = \vect{k}\vect{k}\trans - \vect{I}_3, \quad
\skewsym{\vect{k}}^{3} = -\skewsym{\vect{k}}, \quad
\skewsym{\vect{k}}^{4} = -\skewsym{\vect{k}}^{2}, \quad
\skewsym{\vect{k}}^{5} = \skewsym{\vect{k}}, \quad
\skewsym{\vect{k}}^{6} = \skewsym{\vect{k}}^{2}, \quad
...
&= \vect{I} + \theta\skewsym{\vect{k}} + \tfrac{\theta^2}{2!}\skewsym{\vect{k}}^2 + \tfrac{\theta^3}{3!}\skewsym{\vect{k}}^3 + \tfrac{\theta^4}{4!}\skewsym{\vect{k}}^4 + \tfrac{\theta^5}{5!}\skewsym{\vect{k}}^5 + \tfrac{\theta^6}{6!}\skewsym{\vect{k}}^6 + \tfrac{\theta^7}{7!}\skewsym{\vect{k}}^7 + \cdots \notag\\
&= \vect{I} + \theta\skewsym{\vect{k}} + \tfrac{\theta^2}{2!}\skewsym{\vect{k}}^2 - \tfrac{\theta^3}{3!}\skewsym{\vect{k}} - \tfrac{\theta^4}{4!}\skewsym{\vect{k}}^2 + \tfrac{\theta^5}{5!}\skewsym{\vect{k}} + \tfrac{\theta^6}{6!}\skewsym{\vect{k}}^2 - \tfrac{\theta^7}{7!}\skewsym{\vect{k}} + \cdots \notag\\
&= \vect{I} + \underbrace{\big(\theta - \tfrac{\theta^3}{3!} + \tfrac{\theta^5}{5!} - \tfrac{\theta^7}{7!} + \cdots\big)}_{\sin\theta}\skewsym{\vect{k}}
 + \underbrace{\big(\tfrac{\theta^2}{2!} - \tfrac{\theta^4}{4!} + \tfrac{\theta^6}{6!} - \cdots\big)}_{1-\cos\theta}\skewsym{\vect{k}}^2,
\end{align}
which gives \textbf{Rodrigues' rotation formula}
\begin{empheq}[box=\fbox]{align}
\vect{R}(\vect{\theta}) = \Exp(\vect{\theta}) = \exp(\theta\skewsym{\vect{k}}) = \vect{I} + \sin\theta\,\skewsym{\vect{k}} + (1-\cos\theta)\,\skewsym{\vect{k}}^2.
\end{empheq}
...
\begin{align}
\vect{R}_{1\to k} = \vect{R}_k\vect{R}_{k-1}\cdots\vect{R}_2\vect{R}_1 = \exp(\skewsym{\vect{\omega}_k}\Delta t_k)\cdots\exp(\skewsym{\vect{\omega}_1}\Delta t_1).
\end{align}
...
\begin{align}
{}^{t_1}_{t_2}\vect{R} = {}^{G}_{t_1}\vect{R}\trans\,{}^{G}_{t_2}\vect{R}, \qquad
{}^{t_1}_{t_2}\vect{\theta} = \Log({}^{t_1}_{t_2}\vect{R}) = \Log({}^{G}_{t_1}\vect{R}\trans\,{}^{G}_{t_2}\vect{R}), \qquad
\lambda = \frac{t-t_1}{t_2-t_1}.
\end{align}
Then
\begin{align}
{}^{G}_{t}\vect{R} = {}^{G}_{t_1}\vect{R}\,{}^{t_1}_{t}\vect{R} = {}^{G}_{t_1}\vect{R}\,\Exp\!\big(\lambda\,{}^{t_1}_{t_2}\vect{\theta}\big).
\end{align}
...
\begin{align}
\skewsym{\vect{R}\vect{u}}(\vect{R}\vect{v}) = \vect{R}\big(\skewsym{\vect{u}}\vect{v}\big) = \vect{R}\big(\skewsym{\vect{u}}\vect{R}\trans\vect{R}\vect{v}\big) = \vect{R}\skewsym{\vect{u}}\vect{R}\trans\,(\vect{R}\vect{v}).
\end{align}
...
\subsubsection{Worked applications}
...
\begin{quote}
\textbf{Discussion.} For (11)/(12): What is the minimum number of data pairs needed? What requirements must the pairs satisfy? What if more than the minimum is available?
\end{quote}
...
\begin{align}
\vect{T}_1\vect{T}_2 = \begin{bmatrix}\vect{R}_1 & \vect{p}_1\\\vect{0} & 1\end{bmatrix}\begin{bmatrix}\vect{R}_2 & \vect{p}_2\\\vect{0} & 1\end{bmatrix} = \begin{bmatrix}\vect{R}_1\vect{R}_2 & \vect{R}_1\vect{p}_2+\vect{p}_1\\\vect{0} & 1\end{bmatrix} \in SE(3),
\end{align}
...
\subsubsection{Why this (manifold) machinery?}
...
\begin{tikzpicture}[>=stealth,scale=1.3]
  ...
  \fill (P) circle (1.2pt);
  \node[red,align=center] at ($(P)+(-35:1.55)$) {tangent space\\(error state)};
...
\begin{align}
\vect{R}(\vect{\theta}) = \Exp(\vect{\theta}) = \vect{I}_3 + \skewsym{\vect{\theta}}\vect{J}_l(\vect{\theta}) = \vect{I}_3 + \vect{\theta}^{\wedge}\vect{J}_l(\vect{\theta}), \qquad
...
\paragraph{Exp / Log.} With $\vect{\xi} = \begin{bmatrix}\vect{\theta}\\\vect{\rho}\end{bmatrix}\in\Reals^6$ and the $4\times4$ algebra element $\vect{\xi}^{\wedge} = \begin{bmatrix}\skewsym{\vect{\theta}} & \vect{p}\\ \vect{0}_{1\times3} & 0\end{bmatrix}$,
\begin{align}
\vect{T} = \Exp(\vect{\xi}) = \exp\!\begin{bmatrix}\skewsym{\vect{\theta}} & \vect{p}\\ \vect{0} & 0\end{bmatrix} = \begin{bmatrix} \exp(\skewsym{\vect{\theta}}) & \vect{J}_l(\vect{\theta})\,\vect{p} \\ \vect{0}_{1\times3} & 1 \end{bmatrix} = \begin{bmatrix} \vect{R} & \vect{p}' \\ \vect{0} & 1 \end{bmatrix},
\end{align}
and inverting,
\begin{align}
\vect{\xi} = \Log(\vect{T})^{\vee}, \qquad
\log(\vect{T}) = \log\begin{bmatrix}\vect{R} & \vect{p}'\\ \vect{0} & 1\end{bmatrix} = \begin{bmatrix}\log(\vect{R}) & \vect{J}_l^{-1}(\vect{\theta})\vect{p}'\\ \vect{0} & 0\end{bmatrix}
\;\Rightarrow\;
\vect{\xi} = \begin{bmatrix}\vect{\theta}\\ \vect{J}_l^{-1}(\vect{\theta})\vect{p}'\end{bmatrix}.
\end{align}
\begin{quote}
\textbf{Discussion.} Prove $\exp\!\Big(\begin{bmatrix}\skewsym{\vect{\theta}} & \vect{p}\\ \vect{0} & 0\end{bmatrix}\Big) = \begin{bmatrix}\vect{R} & \vect{J}_l(\vect{\theta})\vect{p}\\ \vect{0} & 1\end{bmatrix}$.
\end{quote}
...
\end{align}
and to first order, with $\vect{\xi}^{\curlywedge} = \begin{bmatrix}\skewsym{\vect{\theta}} & \vect{0}\\ \skewsym{\vect{p}} & \skewsym{\vect{\theta}}\end{bmatrix}_{6\times6}$,
\begin{align}
...
\paragraph{Error states: SE(3) vs SO(3)\,$+\,\Reals^3$.} Perturb $\vect{T}$ on the right by the translation part $\Exp\!\big(\big[\begin{smallmatrix}\vect{0}\\\delta\vect{p}\end{smallmatrix}\big]^{\wedge}\big)$ and rotation part $\Exp\!\big(\big[\begin{smallmatrix}\delta\vect{\theta}\\\vect{0}\end{smallmatrix}\big]^{\wedge}\big)$. Expanding with $\vect{J}_l(\delta\vect{\theta})\approx\vect{I}+\tfrac12\skewsym{\delta\vect{\theta}}$ and $\delta\vect{R} = \Exp(\delta\vect{\theta})$,
\begin{align}
\vect{T}\,\delta\vect{T}
&= \begin{bmatrix}\vect{R}&\vect{p}\\\vect{0}&1\end{bmatrix}\begin{bmatrix}\Exp(\delta\vect{\theta}) & \vect{J}_l(\delta\vect{\theta})\delta\vect{p}\\\vect{0}&1\end{bmatrix}
= \begin{bmatrix}\vect{R}&\vect{p}\\\vect{0}&1\end{bmatrix}\begin{bmatrix}\delta\vect{R} & (\vect{I}+\tfrac12\skewsym{\delta\vect{\theta}})\delta\vect{p}\\\vect{0}&1\end{bmatrix} \notag\\
&\approx \begin{bmatrix}\vect{R}&\vect{p}\\\vect{0}&1\end{bmatrix}\begin{bmatrix}\delta\vect{R} & \delta\vect{p}\\\vect{0}&1\end{bmatrix}
= \begin{bmatrix}\vect{R}\delta\vect{R} & \vect{R}\delta\vect{p}+\vect{p}\\ \vect{0}&1\end{bmatrix}.
\end{align}
If instead we treat the pose as $SO(3)+\Reals^3$ (rotation and translation perturbed separately, $\vect{p}\to\vect{p}+\delta\vect{p}$),
\begin{align}
\vect{T} = \begin{bmatrix}\vect{R}\delta\vect{R} & \vect{p}+\delta\vect{p}\\\vect{0}&1\end{bmatrix} = \begin{bmatrix}\vect{R}\delta\vect{R} & \vect{p}+\vect{R}\vect{R}\trans\delta\vect{p}\\\vect{0}&1\end{bmatrix} = \begin{bmatrix}\vect{R}&\vect{p}\\\vect{0}&1\end{bmatrix}\begin{bmatrix}\delta\vect{R} & \vect{R}\trans\delta\vect{p}\\\vect{0}&1\end{bmatrix}.
\end{align}
Comparing the two conventions gives the relation between the error states,
\begin{align}
...
= \begin{bmatrix}\vect{R}&\vect{p}\\\vect{0}&1\end{bmatrix}\begin{bmatrix}\delta\vect{R}&\vect{R}\trans\delta\vect{p}\\\vect{0}&1\end{bmatrix}_{SE(3)}
\;\Rightarrow\;
\begin{cases} \delta\vect{p}_{SE(3)} = \vect{R}\trans\delta\vect{p} \\ \delta\vect{R}_{SE(3)} = \delta\vect{R} \end{cases}.
\end{align}
...
\paragraph{(2) Time derivative.} For $SE(3)$, with $\vect{T} = \vect{T}\,\Exp\!\big([\delta\vect{\theta};\delta\vect{p}]^{\wedge}\big)$ and body velocity, expand the limit using $\exp(\skewsym{\delta\vect{\theta}})-\vect{I}_3\approx\skewsym{\delta\vect{\theta}}$ and $\vect{J}_l(\delta\vect{\theta})\approx\vect{I}+\tfrac12\skewsym{\delta\vect{\theta}}$:
\begin{align}
\frac{\mathrm{d}\vect{T}}{\mathrm{d}t}
&= \lim_{\delta t\to0}\frac{\vect{T}(t+\delta t)-\vect{T}(t)}{\delta t}
= \lim_{\delta t\to0}\frac{1}{\delta t}\left(\begin{bmatrix}\vect{R}&\vect{p}\\\vect{0}&1\end{bmatrix}\begin{bmatrix}\exp(\skewsym{\delta\vect{\theta}}) & \vect{J}_l(\delta\vect{\theta})\delta\vect{p}\\\vect{0}&1\end{bmatrix} - \begin{bmatrix}\vect{R}&\vect{p}\\\vect{0}&1\end{bmatrix}\right) \notag\\
&= \lim_{\delta t\to0}\frac{1}{\delta t}\begin{bmatrix}\vect{R}&\vect{p}\\\vect{0}&1\end{bmatrix}\begin{bmatrix}\exp(\skewsym{\delta\vect{\theta}})-\vect{I}_3 & \vect{J}_l(\delta\vect{\theta})\delta\vect{p}\\\vect{0}&0\end{bmatrix}
= \lim_{\delta t\to0}\frac{1}{\delta t}\begin{bmatrix}\vect{R}&\vect{p}\\\vect{0}&1\end{bmatrix}\begin{bmatrix}\skewsym{\delta\vect{\theta}} & \delta\vect{p}\\\vect{0}&0\end{bmatrix} \notag\\
...
\end{align}
where $\vect{v}$ is the body velocity. For $SO(3)+\Reals^3$ the same computation with $\vect{T} = \begin{bmatrix}\vect{R}\exp(\skewsym{\delta\vect{\theta}}) & \vect{p}+\delta\vect{p}\\\vect{0}&1\end{bmatrix}$ gives
\begin{align}
...
\vect{\xi} = \begin{bmatrix}\vect{\theta}\\\vect{p}\\\vect{v}\end{bmatrix} \to \vect{\xi}^{\wedge} = \begin{bmatrix}\skewsym{\vect{\theta}} & \vect{p} & \vect{v} \\ \vect{0} & 0 & 0 \\ \vect{0} & 0 & 0\end{bmatrix}_{5\times5}.
\end{align}
The exponential, logarithm, and adjoint follow the same pattern as $SE(3)$:
\begin{align}
\Exp(\vect{\xi}) = \begin{bmatrix} \vect{R} & \vect{J}_l(\vect{\theta})\vect{p} & \vect{J}_l(\vect{\theta})\vect{v} \\ \vect{0} & 1 & 0 \\ \vect{0} & 0 & 1\end{bmatrix}, \quad
\Log(\vect{\Upsilon})^{\vee} = \begin{bmatrix}\vect{\theta}\\ \vect{J}_l^{-1}(\vect{\theta})\vect{p}\\ \vect{J}_l^{-1}(\vect{\theta})\vect{v}\end{bmatrix}, \quad
...
\begin{align}
\bar{p}\otimes\bar{q} = \mathcal{L}(\bar{p})\,\bar{q} = \mathcal{R}(\bar{q})\,\bar{p}, \quad
\mathcal{L}(\bar{p}) = p_w\vect{I}_4 + \begin{bmatrix} 0 & -\vect{p}_v\trans \\ \vect{p}_v & \skewsym{\vect{p}_v} \end{bmatrix}, \quad
...
\begin{align}
\begin{bmatrix}0\\ {}^{G}\vect{p}\end{bmatrix} = {}^{G}_{I}\bar{q}\otimes\begin{bmatrix}0\\ {}^{I}\vect{p}\end{bmatrix}\otimes{}^{G}_{I}\bar{q}^{-1}
...
\begin{align}
q_w^2\vect{I}_3 + \vect{q}_v\vect{q}_v\trans + \skewsym{\vect{q}_v}^2 = q_w^2\vect{I}_3 + (\vect{q}_v\trans\vect{q}_v)\vect{I}_3 + 2\skewsym{\vect{q}_v}^2 = \vect{I}_3 + 2\skewsym{\vect{q}_v}^2,
\end{align}
so
\begin{align}
...
\begin{align}
{}^{G}\vect{p} = \big(\vect{I}_3 + \sin\theta\,\skewsym{\vect{k}} + (1-\cos\theta)\skewsym{\vect{k}}^2\big){}^{I}\vect{p} = {}^{G}_{I}\vect{R}\,{}^{I}\vect{p} = \Exp(\theta\vect{k})\,{}^{I}\vect{p},
\end{align}
